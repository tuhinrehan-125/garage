...
export default {
  data() {
    return {
      valid: true,
      nameRules: [(v) => !!v || this.$t("Color name is required")],
      form: {
        name: "",
        description: "",
      },
  ...
  methods: {
    ...
    async submitForm() {
      if (this.$refs.form.validate()) {
        await this.$axios.post("/vehicle-color", this.form).then((res) => {
          this.$refs.form.reset();
          let data = { alert: true, message: "Color Added Successfully",type:'success' };
